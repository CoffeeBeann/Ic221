/***************************************************
Filename: unroll.c
Author: MIDN Ian Coffey (m261194) ***(Received Assistance From MIDN 3/C IKER)***
To Unroll A Piped Command Into Sepate Fork & Exec Cmds
***************************************************/
...
int main(int argc, char * argv[]){
	...
	for (i = 0; pipeline[i][0] != NULL; i++) {
		...
		if (cpid == 0) {	
			...
			execvp(pipeline[i][0], pipeline[i]); // Run Commdand
			...
			if (i == 0) { // if first child:
				...
				setpgid(cpid, cpid); // CPID GRPIN = CPID
